...
function guardarPedido(tx){
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS Pedidos (id INTEGER PRIMARY KEY AUTOINCREMENT, fecha TEXT, user TEXT, cod_asesor TEXT, cliente TEXT, codigo TEXT, local TEXT, email TEXT, observaciones TEXT '+lista_productos+')');

    tx.executeSql('INSERT INTO Pedidos (fecha, user, cod_asesor, cliente, codigo, local, email, observaciones '+lista_productos1+') VALUES ("'+fecha+'", "'+asesor+'", "'+asesor+'", "'+$("#cliente").val()+'", "'+$("#codigo").val()+'", "'+$("#local").val()+'", "'+$("#email").val()+'", "'+$("#observaciones").val()+'" '+lista_productos2+')');
